...
// Converts custom '#schema Category,Score,Comments\nValue1,Value2,Value3' output format
// generated by Gemini to standard valid TOON array format '[length]{keys}:'
// and handles potential inline commas in text columns safely.
const convertCustomToonToStandard = (customToon: string): string => {
  const lines = customToon.trim().split('\n').map(l => l.trim()).filter(Boolean);
  ...
  const fieldsStr = firstLine.replace('#schema ', '').trim();
  const fields = fieldsStr.split(',').map(f => f.trim());
  const schemaCount = fields.length;
  
  const dataLines = lines.slice(1);
  const sanitizedDataLines = dataLines.map(line => {
    // If the line is already fully quoted, leave it
    ...
    if (parts.length <= schemaCount) {
      return line;
    }
    
    // Merge extra comma parts into the final field (e.g. Comments)
    const leadingParts = parts.slice(0, schemaCount - 1);
    const trailingParts = parts.slice(schemaCount - 1);
    const escapedTrailing = `"${trailingParts.join(',').replace(/"/g, '\\"')}"`;
    return [...leadingParts, escapedTrailing].join(',');
  });
  
  const length = sanitizedDataLines.length;
  const standardLines = [
    `[${length}]{${fieldsStr}}:`,
    ...sanitizedDataLines.map(line => `  ${line}`)
  ];
  ...
  const parseMessageText = (text: string, role: 'user' | 'model') => {
    ...
    if (role === 'model') {
      const toonRegex = /```(?:toon|[a-zA-Z0-9_-]*)?\s*(#schema[\s\S]*?|\[\d+\]\{[\s\S]*?)```/g;
